...
const STAGES: PipelineStage[] = [
  {
    ...
    tasks: ["LinkedIn scraping", "Twitter/X posts", "crunchbase funding", "jobs boards scan", "website changelogs"],
  ...
  },
  {
    ...
    tasks: ["Buying / Hiring intent", "Partnership signals", "Expansion flags", "Creator collab checks", "Urgency index"],
  ...
  },
  {
    ...
    tasks: ["Lead scoring (0-100)", "Convert probability", "Urgency verification", "Deal size estimation"],
  ...
  },
  {
    ...
    tasks: ["Outbound email drafts", "LinkedIn InMails", "CRM deal entries", "Slack team notifications"],
  ...
  },
  {
    ...
    tasks: ["Opportunity heatmaps", "Funnel charts", "Pipeline velocities", "Stakeholder PDF reports"],
...
const SIMULATION_LOGS = [
  ...
  { step: 3, text: "Prioritizing leads: Vercel score = 92 (CRITICAL). Estimated deal size: $1,380." },
  ...
  { step: 4, text: "CRM: HubSpot deal record inserted for Vercel Series C. Stage: Appointment Scheduled." },
  ...
  return (
    <ParallaxField className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs uppercase tracking-widest text-primary">Agentic Workflow Topology</p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tight">AI Pipeline Architecture</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Understand how our autonomous agent swarm scrapes signals, classifies intent, prioritizes hot deals, and triggers actions.
          </p>
        </div>
        <button
              ...
              <div key={s.step} className="flex-1 flex items-start relative z-10">
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-2.5 mb-2.5">
                    <span className="font-mono text-xs font-semibold text-muted-foreground">STAGE {s.step}</span>
                    <div className={`p-1.5 rounded-md ${active ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}>
                  ...
                  <ul className="mt-3.5 space-y-1.5 text-left">
                    {s.tasks.map((t) => (
                      <li key={t} className="text-[10px] text-muted-foreground flex items-center gap-1">
                        <span className={`h-1 w-1 rounded-full ${active ? "bg-primary animate-ping" : "bg-muted"}`} />
                        {t}
                      </li>
                    ))}
                  </ul>
      ...
      <DepthLayer className="rounded-2xl p-5 space-y-4">
        ...
        <div className="bg-background/80 rounded-xl border border-border/80 p-4 font-mono text-xs text-left h-64 overflow-y-auto space-y-2">
          {logs.length === 0 ? (
            ...
            logs.map((l, i) => (
              <div key={i} className="text-muted-foreground hover:text-foreground transition duration-150">
        ...
        <div className="flex justify-between items-center text-[10px] text-muted-foreground pt-1">
          <span className="flex items-center gap-1">
            <ShieldCheck className="h-3.5 w-3.5 text-success" /> Pipeline is secure and GDPR/SOC-2 compliant
          </span>
