...
function mapFirebaseError(error: unknown): string {
  ...
  const map: Record<string, string> = {
    ...
    "auth/popup-blocked":
      "Popup was blocked by the browser. Allow popups for this site.",
    "auth/cancelled-popup-request":
      "Only one sign-in window at a time. Please try again.",
    "auth/account-exists-with-different-credential":
      "An account with this email already exists using a different sign-in method.",
    "auth/invalid-credential":
      "Your credentials are invalid or have expired. Please try again.",
  ...
  return (
    <span
      ...
      style={{
        ...
        height: 6,
        borderRadius: "50%",
        background: "oklch(0.78 0.16 205 / 0.6)",
        boxShadow: "0 0 10px 2px oklch(0.78 0.16 205 / 0.5)",
  ...
  return (
    <span
      ...
      style={{
        ...
        borderTopColor: "oklch(0.16 0.03 260)",
...
function AuthPage() {
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          console.warn("[Supabase] Email already registered in Supabase — deleting orphaned Firebase account.");
          if (firebaseUser) {
            try {
          ...
          const conflictMsg = "This email address is already registered. Please sign in with your email and password instead.";
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      console.warn("[Supabase] Mirror sign-in failed. Attempting to register new Supabase account...");
      const { data: signupData, error: signupErr } = await supabase.auth.signUp({
        email: user.email!,
        password: user.uid,
        options: {
      ...
      if (signupData?.user && !signupData?.session) {
        console.log("[Supabase] Account created but no session — email confirmation may be required.");
      ...
      console.error("[OAuth] Unexpected state — no session and no error. signInData:", signInData, "signupData:", signupData);
      toast.error("Google authentication failed", {
        description: "An unexpected error occurred. Please try again or sign in with email/password.",
      });
  ...
  return (
    <>
      <style>{`
        ...
        .auth-input:focus {
          border-color: oklch(0.78 0.16 205);
          box-shadow: 0 0 0 3px oklch(0.78 0.16 205 / 0.12);
        }
        .auth-input.has-error {
          border-color: oklch(0.65 0.24 25);
          box-shadow: 0 0 0 3px oklch(0.65 0.24 25 / 0.12);
        }
        .auth-input::placeholder { color: oklch(0.42 0.02 250); }
        ...
        .auth-btn-primary {
          ...
          border-radius: 10px;
          border: none;
          background: oklch(0.78 0.16 205);
          color: oklch(0.16 0.03 260);
          font-size: 13.5px;
          font-weight: 700;
          letter-spacing: 0.01em;
          cursor: pointer;
          box-shadow: 0 0 24px -4px oklch(0.78 0.16 205 / 0.45);
        ...
        .auth-btn-google:hover:not(:disabled) {
          background: oklch(0.26 0.04 260);
          border-color: oklch(0.78 0.16 205 / 0.4);
      ...
      >
        ...
        <div aria-hidden style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(ellipse 60% 50% at 20% 40%, oklch(0.4 0.14 205 / 0.18), transparent 60%), radial-gradient(ellipse 50% 60% at 80% 70%, oklch(0.32 0.12 285 / 0.22), transparent 60%)", pointerEvents: "none" }} />
        <div aria-hidden style={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(oklch(0.3 0.03 260 / 0.35) 1px, transparent 1px), linear-gradient(90deg, oklch(0.3 0.03 260 / 0.35) 1px, transparent 1px)", backgroundSize: "32px 32px", pointerEvents: "none", opacity: 0.5 }} />

        {mounted && (
          <>
            <SignalDot style={{ top: "12%", left: "8%", animationDelay: "0s" }} />
            <SignalDot style={{ top: "70%", left: "15%", animationDelay: "1.5s" }} />
            <SignalDot style={{ top: "35%", left: "40%", animationDelay: "0.8s", background: "oklch(0.68 0.22 300 / 0.5)" }} />
            <SignalDot style={{ top: "80%", left: "60%", animationDelay: "2.2s" }} />
            <SignalDot style={{ top: "20%", right: "12%", animationDelay: "0.3s" }} />
            <SignalDot style={{ top: "55%", right: "8%", animationDelay: "3s", background: "oklch(0.75 0.17 155 / 0.6)" }} />
          </>
        )}
        ...
        >
          <Link to="/" style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: "3.5rem", textDecoration: "none" }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: "oklch(0.78 0.16 205 / 0.15)", border: "1px solid oklch(0.78 0.16 205 / 0.3)", display: "grid", placeItems: "center", color: "oklch(0.78 0.16 205)", boxShadow: "0 0 20px -4px oklch(0.78 0.16 205 / 0.4)" }}>
              <Radar size={20} />
            </div>
            <span style={{ fontWeight: 700, fontSize: 20, letterSpacing: "-0.02em", background: "linear-gradient(135deg, oklch(0.85 0.16 205), oklch(0.72 0.22 300))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
              Signal Scout
            </span>
          </Link>

          <h1 style={{ fontSize: "clamp(2rem, 3.5vw, 2.8rem)", fontWeight: 700, lineHeight: 1.15, letterSpacing: "-0.03em", color: "oklch(0.97 0.01 240)", marginBottom: "1rem" }}>
            Revenue intelligence,{" "}
            <span style={{ background: "linear-gradient(135deg, oklch(0.85 0.16 205), oklch(0.72 0.22 300))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>automated.</span>
          </h1>
          <p style={{ fontSize: 15, color: "oklch(0.68 0.02 250)", lineHeight: 1.65, maxWidth: 380, marginBottom: "2.5rem" }}>
            Deploy autonomous agents that harvest signals from LinkedIn, Twitter, funding news and job boards — then surface the accounts ready to buy.
          </p>

          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 3rem 0", display: "flex", flexDirection: "column", gap: 14 }}>
            {FEATURES.map(({ icon: Icon, label, color }) => (
              <li key={label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: `${color}18`, border: `1px solid ${color}30`, display: "grid", placeItems: "center", color, flexShrink: 0 }}>
                  <Icon size={14} />
                </div>
                <span style={{ fontSize: 13.5, color: "oklch(0.82 0.02 250)" }}>{label}</span>
              </li>
            ))}
          </ul>
          ...
          <div style={{ display: "flex", gap: 32 }}>
            {STATS.map(({ value, label }) => (
              <div key={label}>
                <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.03em", background: "linear-gradient(135deg, oklch(0.85 0.16 205), oklch(0.72 0.22 300))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>{value}</div>
                <div style={{ fontSize: 11, color: "oklch(0.55 0.02 250)", marginTop: 2 }}>{label}</div>
              </div>
            ))}
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: "3rem" }}>
            <Shield size={13} style={{ color: "oklch(0.75 0.17 155)" }} />
            <span style={{ fontSize: 11.5, color: "oklch(0.55 0.02 250)" }}>SOC 2 ready · Firebase Auth · End-to-end encrypted</span>
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", position: "relative", zIndex: 1 }}>
          <div
            ...
            style={{
              ...
              borderRadius: 20,
              border: "1px solid oklch(0.3 0.03 260)",
              background: "oklch(0.19 0.035 260 / 0.88)",
              backdropFilter: "blur(24px)",
              padding: "2.5rem",
              boxShadow: "0 0 60px -12px oklch(0.72 0.19 210 / 0.25), 0 32px 64px -24px oklch(0 0 0 / 0.5)",
            }}
          >
            {/* Tab switcher */}
            <div style={{ display: "flex", background: "oklch(0.16 0.03 260)", borderRadius: 10, padding: 4, marginBottom: "1.75rem", border: "1px solid oklch(0.28 0.03 260)" }}>
              {(["signin", "signup"] as const).map((m) => (
                <button
                  ...
                  style={{
                    ...
                    background: mode === m ? "oklch(0.78 0.16 205)" : "transparent",
                    color: mode === m ? "oklch(0.16 0.03 260)" : "oklch(0.55 0.02 250)",
                    boxShadow: mode === m ? "0 0 16px -4px oklch(0.78 0.16 205 / 0.5)" : "none",
                    fontFamily: "inherit",
                  }}
                >
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              <h2 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", color: "oklch(0.97 0.01 240)", margin: 0 }}>
                {mode === "signin" ? "Welcome back" : "Start for free"}
              </h2>
              <p style={{ fontSize: 13, color: "oklch(0.55 0.02 250)", marginTop: 4 }}>
                {mode === "signin"
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "1.25rem 0" }}>
              <div style={{ flex: 1, height: 1, background: "oklch(0.28 0.03 260)" }} />
              <span style={{ fontSize: 11, color: "oklch(0.45 0.02 250)", letterSpacing: "0.08em" }}>OR</span>
              <div style={{ flex: 1, height: 1, background: "oklch(0.28 0.03 260)" }} />
            </div>
            ...
            {fieldError && (
              ...
              >
                <AlertTriangle size={14} style={{ color: "oklch(0.75 0.22 25)", flexShrink: 0, marginTop: 1 }} />
            ...
            <form id="auth-form" onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {mode === "signup" && (
                <div className="field-slide">
                  <label htmlFor="auth-username" style={{ display: "block", fontSize: 12, color: "oklch(0.65 0.02 250)", marginBottom: 5, fontWeight: 500 }}>
                    Full name <span style={{ color: "oklch(0.65 0.24 25)" }}>*</span>
                  </label>
                  <div style={{ position: "relative" }}>
                    <Users size={15} aria-hidden style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "oklch(0.5 0.02 250)", pointerEvents: "none" }} />
                    <input
                      id="auth-username"
              ...
              {/* Email */}
              <div>
                <label htmlFor="auth-email" style={{ display: "block", fontSize: 12, color: "oklch(0.65 0.02 250)", marginBottom: 5, fontWeight: 500 }}>
                  Work email <span style={{ color: "oklch(0.65 0.24 25)" }}>*</span>
                </label>
                <div style={{ position: "relative" }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "oklch(0.5 0.02 250)", pointerEvents: "none" }}>
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                  <label htmlFor="auth-password" style={{ fontSize: 12, color: "oklch(0.65 0.02 250)", fontWeight: 500 }}>
                    Password <span style={{ color: "oklch(0.65 0.24 25)" }}>*</span>
                  </label>
                  {mode === "signin" && (
                    <button
                      type="button"
                      style={{ background: "none", border: "none", fontSize: 11.5, color: "oklch(0.78 0.16 205)", cursor: "pointer", fontFamily: "inherit", padding: 0 }}
                      onClick={() => toast.info("Password reset", { description: "Enter your email and we'll send a reset link." })}
                    >
                      Forgot password?
                    </button>
                  )}
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <button
                    ...
                    style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "oklch(0.5 0.02 250)", display: "grid", placeItems: "center", padding: 4 }}
                  >
                ...
                {mode === "signup" && password.length > 0 && (
                  <div style={{ display: "flex", gap: 4, marginTop: 6 }} role="presentation" aria-label="Password strength">
                    {[1, 2, 3, 4].map((n) => {
                      const strength =
                        password.length < 6 ? 1 : password.length < 8 ? 2 : password.length < 12 ? 3 : 4;
                      const active = n <= strength;
                      const color = strength === 1 ? "#f87171" : strength === 2 ? "#fbbf24" : strength === 3 ? "#34d399" : "#22d3ee";
                      return (
                        <div key={n} style={{ flex: 1, height: 3, borderRadius: 99, background: active ? color : "oklch(0.28 0.03 260)", transition: "background 0.3s ease" }} />
                      );
                    })}
                  </div>
                )}
            ...
            <div style={{ marginTop: 18, textAlign: "center" }}>
              {mode === "signup" && (
                <p style={{ fontSize: 11, color: "oklch(0.45 0.02 250)", marginBottom: 10, lineHeight: 1.5 }}>
              ...
              <button
                ...
                onClick={switchMode}
                disabled={anyLoading}
                style={{ background: "none", border: "none", fontSize: 12.5, color: "oklch(0.55 0.02 250)", cursor: "pointer", transition: "color 0.2s ease", fontFamily: "inherit" }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "oklch(0.78 0.16 205)"; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "oklch(0.55 0.02 250)"; }}
              >
                {mode === "signin"
            ...
            {/* Trust badges */}
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid oklch(0.24 0.03 260)", display: "flex", justifyContent: "center", gap: 20 }}>
              {[
                { icon: CheckCircle2, label: "Firebase Auth" },
                { icon: Shield, label: "256-bit TLS" },
                { icon: TrendingUp, label: "99.9% uptime" },
              ].map(({ icon: Icon, label }) => (
                <div key={label} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "oklch(0.45 0.02 250)" }}>
                  <Icon size={11} style={{ color: "oklch(0.75 0.17 155)" }} />
                  {label}
                </div>
              ))}
            </div>
