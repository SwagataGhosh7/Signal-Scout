...
export function AiAssistant() {
  ...
  const handleSend = (textToSend?: string) => {
    ...
    setTimeout(() => {
      let response =
        "I'm checking the signal databases for that now. Let me know if you want me to queue a HubSpot sync.";
      const q = query.toLowerCase();

      if (q.includes("buying") || q.includes("intent")) {
        response =
          "I have detected 3 companies with HIGH Buying Intent: Vercel ($40M Series C funding), Supabase (hiring sales engineers), and Stripe (integrating OpenAI models). I suggest drafting an Executive Outreach script for Vercel immediately.";
      } else if (q.includes("hiring")) {
        response =
          "Supabase has posted 4 new positions for 'Sales Engineer' and 'Enterprise Solutions Architect' on LinkedIn. This indicates strong hiring intent and structural scaling. Would you like me to draft an outreach note targeting their Head of Sales?";
      } else if (q.includes("outreach") || q.includes("email") || q.includes("script")) {
        response =
          "Sure, I have already prepared email templates in the 'Outreach' page. I recommend using the 'Friendly' tone when pitching startups or 'Executive' tone for Series C entities.";
      } else if (q.includes("summary") || q.includes("scans") || q.includes("today")) {
        response =
          "Signal Scout agents completed 14 target scans today. They harvested 8 new signals (4 hiring, 2 buying, 2 product updates). Average opportunity score calculated: 78%.";
      }
